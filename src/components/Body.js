...
const Body = ({titles, posts}) => {
    ...
    return (
        <>
            ...
            <Switch>
                {titles.map((title) => title.slug === "home" ? <Route path={`/`} exact children={<HomeBody titleID={title.id} posts={posts}/>}/> : <Route path={`/${title.slug}/:id`} exact children={<ShowContent titleID={title.id} titleIDChildren={title.children} posts={posts}/>}/>)}
